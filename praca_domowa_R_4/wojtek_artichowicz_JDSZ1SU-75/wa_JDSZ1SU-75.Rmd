...
* Only fast and expensive cars have number of cylinders equal to number of carburetors;
...
#### Based on CA of cylinders vs. gears
* Most cars have 8 cylinders and 3 gears, or 4 cylinders with 4 gears - for some reason those are typical constructions;
* Least significant technical solution is 6 cylinders and 5 gears - untypical / unpopular?;
*  The strong data variability exists only in PCA1 direction, that may indicate that there is strong dependence of number of gears and cylinders.
...
Variables (for example car entities) plotted on such plot depict the poistion of that points in the new features space projected onto plane of chosen directions. If points are close they are somewhat similar, if they are far they significantly different.

Scree plot shows the amount of information in each of the new variables.For example here ~69% of the total dataset variability is "compressed" into first PCA.
...
CA biplot shows that all engine configurations are far one from another except the ones that have 3 or 8 carburetors and 8 cylinders.
...
Balloonplot shows that most cars have 8 cylinders and 3 gears, another popular setting is 4 cylinders with 4 gears.

Contribution plots show that the least variability is delivered by 6 cylinders and 5 gears. Variability in the second direction is neglectible. The strong data variability exists only in PCA1 direction. Points denoting 8 cylinders and 3 gears are close, which means they are commonly used in common. Similarly points denoting 4 gears and 4 cylinders are close. That shows that these are the typcal constructions.
