...
select typ_podrozy, count(1) as liczba,
      count(1) / sum(count(1)) over()::numeric procent
from wnioski
...
select to_char(data_otrzymania, 'YYYY-MM-DD'), sum(kwota)
from szczegoly_rekompensat
GROUP BY 1
ORDER BY 2 DESC;
...
from wnioski w
  join podroze p on w.id = p.id_wniosku
  join szczegoly_podrozy s2 ON p.id = s2.id_podrozy
  where extract(days from w.data_utworzenia - s2.data_wyjazdu) > 1095
group by 1;
...
from wnioski w
  join podroze p on w.id = p.id_wniosku
  join szczegoly_podrozy s2 ON p.id = s2.id_podrozy
  where w.data_utworzenia- data_wyjazdu > '3 years'
group by 1;
...
select  id_wniosku, identyfikator_podrozy,
    first_value(kod_wyjazdu) over(PARTITION BY id_wniosku order by data_wyjazdu asc) wylot,
    first_value(kod_przyjazdu) over(PARTITION BY id_wniosku order by data_wyjazdu desc) powrot
from wnioski w
join podroze p ON w.id = p.id_wniosku
join szczegoly_podrozy s2 ON p.id = s2.id_podrozy
where s2.id_podrozy in (126400,127857)

update szczegoly_podrozy
set kod_wyjazdu = 'WAW' where id = 148124



/* klienci którzy mają więcej niż dwa języki, porównanie first i last */



with moje_daty as (select generate_series(
  date_trunc('month', now()-interval '1 month'),
  date_trunc('month', now())+interval '1 month'-interval '1 day',
  '1 day'
)::date as wyg_data),

aktualne_wnioski as (select to_char(data_utworzenia, 'YYYY-MM-DD')::date as data_wnioskow, count(1) as liczba_wnioskow
FROM wnioski
...
poprzednie_3_tyg as (
    select *
    from aktualne_wnioski
    where data_wnioskow > '2018-02-09'::date-interval '3 weeks' + interval '1 day'
  )

select md.wyg_data, to_char(md.wyg_data, 'day'),
    avg(coalesce(liczba_wnioskow, 0)) srednia
from moje_daty md
left join poprzednie_3_tyg p on p.data_wnioskow = md.wyg_data
group by 1;



select md.wyg_data, aw.liczba_wnioskow, sum(aw.liczba_wnioskow) over(ORDER BY md.wyg_data)
FROM  moje_daty md
left join aktualne_wnioski aw on aw.data_wnioskow = md.wyg_data;

/* zadanie dodatkowe  nr. 1 */

with wspolpasazerowie as (
select wspolpasazerowie.email, identyfikator_podrozy, count(identyfikator_podrozy) over (PARTITION BY wspolpasazerowie)
from wspolpasazerowie
join wnioski w2 ON wspolpasazerowie.id_wniosku = w2.id
join podroze p ON w2.id = p.id_wniosku
join szczegoly_podrozy s2 ON p.id = s2.id_podrozy
join klienci k ON w2.id = k.id_wniosku
 WHERE identyfikator_podrozy not like '%--%' and wspolpasazerowie.email = k.email
order by 3 desc ),


klienci as (select klienci.email, identyfikator_podrozy, count(identyfikator_podrozy) over (PARTITION BY klienci)
from klienci
join wnioski w2 ON klienci.id_wniosku = w2.id
join podroze p ON w2.id = p.id_wniosku
join szczegoly_podrozy s2 ON p.id = s2.id_podrozy
join wspolpasazerowie w3 ON w2.id = w3.id_wniosku
WHERE identyfikator_podrozy not like '%--%' and klienci.email = w3.email
order by 3 desc)


/* zadanie dodatkowe  nr. 2 */

with odrzucony_wniosek as (
    select wnioski.id as odrzucone,  stan_wniosku, identyfikator_podrozy, wnioski.data_utworzenia
  from wnioski
    join podroze p2 ON wnioski.id = p2.id_wniosku
    join szczegoly_podrozy p3 ON p2.id = p3.id_podrozy
where stan_wniosku in ('odrzucony po analizie', 'odrzucony prawnie' , 'odrzucony przez operatora') and identyfikator_podrozy not like '%--%'),


wyplacony_wniosek as (
     select wnioski.id as wyplacone,  stan_wniosku, identyfikator_podrozy, data_zakonczenia
  from wnioski
    join podroze p2 ON wnioski.id = p2.id_wniosku
    join szczegoly_podrozy p3 ON p2.id = p3.id_podrozy
    join rekompensaty r ON wnioski.id = r.id_wniosku
where stan_wniosku in ('wyplacony') and identyfikator_podrozy not like '%--%')
